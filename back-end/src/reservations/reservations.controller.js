 ...
 *  Lists reservations by query data, query mobile_number, or all reservations if no filter is provided
 */
async function list(req, res) {
  console.log("📌 API Request Received:", req.query); // ✅ Debugging log

  const { date, mobile_number } = req.query;

  if (date) {
    const data = await service.list(date);
    console.log("📌 Sending filtered reservations:", data);
    return res.json({ data });
  }

  if (mobile_number) {
    const data = await service.search(mobile_number);
    console.log("📌 Sending search results:", data);
    return res.json({ data });
  }

  // ✅ If no filters, return ALL reservations
  const data = await service.list();
  console.log("📌 Sending ALL reservations:", data);
  res.json({ data });
}
...
module.exports = {
  ...
  update: [  // ✅ Add this to fix the issue
    hasData,
    asyncErrorBoundary(reservationExists),
    hasOnlyValidProperties,
    hasProperties("first_name", "last_name", "mobile_number", "reservation_date", "reservation_time", "people"),
